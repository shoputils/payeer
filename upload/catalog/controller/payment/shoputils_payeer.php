...
class ControllerPaymentShoputilsPayeer extends Controller {
    private $order;
    private $log;
    private $version;
    ...
    public function __construct($registry) {
        ...
        $this->load->language('payment/shoputils_payeer');
    }
    ...
    public function index() {
        ...
        if (version_compare(VERSION, '2.2.0.0', '<')) {
            return is_file(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/shoputils_payeer.tpl')
                    ? $this->load->view($this->config->get('config_template') . '/template/payment/shoputils_payeer.tpl', $this->data)
                    : $this->load->view('default/template/payment/shoputils_payeer.tpl', $this->data);
        } else {
            return $this->load->view('payment/shoputils_payeer', $this->data);
        }
    }
    ...
    public function laterpay() {
        ...
        $this->_setData(
            ...
            if (version_compare(VERSION, '2.2.0.0', '<')) {
                $this->response->setOutput(is_file(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/shoputils_payeer_laterpay.tpl')
                                          ? $this->load->view($this->config->get('config_template') . '/template/payment/shoputils_payeer_laterpay.tpl', $this->data)
                                          : $this->load->view('default/template/payment/shoputils_payeer_laterpay.tpl', $this->data));
            } else {
                $this->response->setOutput($this->load->view('payment/shoputils_payeer_laterpay.tpl', $this->data));
            }
        } else {
    ...
    protected function sendMail($subject, $content, $order_status_id, $type = 'admin', $log_result = '') {
				$this->load->model('payment/shoputils_payeer');

				$order_info = $this->model_payment_shoputils_payeer->getOrder($this->order['order_id']);
            ...
            'order_id'            => $order_info['order_id'],
				    ...
				    'logo'                => '<a href="' . HTTP_SERVER . '"><img src="' . HTTP_SERVER . 'image/' . $this->config->get('config_logo') . '" / ></a>',
						//'products'            => $this->model_payment_shoputils_payeer->getProducts($order_info['order_id']),
						...
						'customer_group'      => $this->model_payment_shoputils_payeer->getCustomerGroup($order_info['customer_group_id']),
						'customer_email'      => $order_info['email'],
						'customer_telephone'  => $order_info['telephone'],
						'order_status'        => $this->model_payment_shoputils_payeer->getOrderStatusById($order_status_id, $order_info['language_id']),
